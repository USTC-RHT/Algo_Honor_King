...
class Algo(BaseAlgo):
    ...
    def learn(self, list_sample_data):
        ...
        with torch.no_grad():
            ...
            target_a_noise = (torch.randn_like(action) * self.target_policy_noise).clamp(-self.target_policy_noise_clip, self.target_policy_noise_clip)
            ''' Target Policy Smoothing Regularization '''
            smoothed_target_action = (self.Actor_Target(next_state) + target_a_noise).clamp(-self.max_action, self.max_action)
        ...
        if self.delay_count % self.config.delay_freq == 0:
            '''actor loss'''
            ...
            return None, critic_loss.detach().cpu().numpy() 
